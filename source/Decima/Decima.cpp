...
namespace Decima
{
	void FileHeader::Decrypt()
	{
		...
		_mm_storeu_si128((__m128i*)&(((std::uint32_t*)this)[2]), CurVec);


		CurVec = _mm_blend_epi16(
			MurmurSaltV,
			_mm_set1_epi32(this->Version + 1),
			3
		);
		CurHashInput = CurVec;
		MurmurHash3_x64_128(&CurHashInput, 0x10, Decima::MurmurSeed, &CurHash);
		CurVec = _mm_xor_si128(
			_mm_loadu_si128((__m128i*)&(((std::uint32_t*)this)[2 + 4])),
			CurHash
		);
		_mm_storeu_si128((__m128i*)&(((std::uint32_t*)this)[2 + 4]), CurVec);
	}
